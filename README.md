...
| Datastore | Not a standalone entity. Referenced by name from a Policy's `datastore` field; its `config` is merged into that policy's plugin config at a path fixed by the plugin's type (`config.redis` for most; `config.storage_config.redis` for `acme`; `config.resources.cache.redis` for `datakit`; `config.vectordb.<redis\|pgvector>` for the VectorDB family, sub-block and `strategy` chosen by the Datastore's `type`). Rejected if the plugin type doesn't support a Datastore, or if the Datastore's `type` isn't one that plugin accepts. |
