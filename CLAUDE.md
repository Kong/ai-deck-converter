...
- A semantic-balancer model's `config.balancer.vectordb` is lowered via `aimap.VectorDBToPlugin`, then a `datastores` reference (if any) is substituted into it by `resolveModelDatastore`/`modelVectorDB` — see "Datastore support" below.
...
A `Datastore` (`internal/aigw/datastore.go`) is a shared connection (`redis-ce`/`redis-ee`/`vectordb`) declared at the top level of the document and referenced by name — `datastores: [{name: x}]` — from a Policy or from a semantic-balancer Model. `internal/aimap/datastore.go` owns every rule about the pairing, shared by both callers: `ApplyDatastore` (policies) checks plugin-type support and cardinality, then substitutes the connection at a plugin-specific `ConfigPath`; `ApplyModelDatastore` (models) restricts to `redis-ee`/`vectordb` and substitutes into `config.vectordb`'s strategy sub-block — always *after* `aimap.VectorDBToPlugin` has lowered that block, since a Datastore's `config` is already plugin-shaped. `convert/policy.go`'s `applyDatastore` and `convert/model.go`'s `resolveModelDatastore`/`modelVectorDB` are the only callers, and they alone decide warn-vs-fail on a dangling reference (aimap returns `*aimap.UnknownDatastoreError` and stays -strict-agnostic). Both entity kinds behave identically: an unknown name calls `c.warn`, which only becomes an error under `-strict`, so it otherwise converts without the connection; every other invalid reference — more than one Datastore, a non-semantic balancer, a disallowed type — always hard-fails via `c.failAt` regardless of `-strict`. `type: api` models sharing one route-scoped `ai-proxy-advanced` (see `convertModels` above) cannot carry conflicting datastores — `convertModels` rejects that combination outright rather than silently dropping one model's datastore or having another model inherit it. Datastores are convert-only: `revert/` inlines the substituted connection straight into the entity's `config` and never reconstructs a Datastore or its reference, which is why `revert/roundtrip_test.go` still passes byte-identical for datastore-bearing golden cases.
