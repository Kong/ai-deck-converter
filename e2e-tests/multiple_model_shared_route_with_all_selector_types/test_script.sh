...
# Step 5: send a proxy request(s)


# using default body selector with "model" property
echo "sending proxy request using default body selector..."
...
# using custom body selector
echo "sending proxy request using body selector..."
curl -X POST http://localhost:8000/ai/chat/completions \
  -H 'Content-Type: application/json' \
  -d '{
        "my-model-alias": "my-body-alias-value",
        "messages": [{"role": "user", "content": "Say hello in one word."}]
      }'
echo "sending proxy request using header selector..."
curl -X POST http://localhost:8000/ai/chat/completions \
  -H 'Content-Type: application/json' \
  -H 'my-model-alias: my-model-alias-header-value' \
  -d '{
        "model": "gpt-99999999",
        "messages": [{"role": "user", "content": "Say hello in one word."}]
      }'
...
# using path selector
echo "sending proxy request using a path selector..."
